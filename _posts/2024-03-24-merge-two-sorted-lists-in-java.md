---
title: "Merge Two Sorted Lists in Java"
date: '2024-03-24'
categories:
- DSA
- Java
...
toc_icon: cog
seo_title: "Merge Two Sorted Lists in Java"
seo_description: "Understand merge two sorted lists in Java with a clean Java walkthrough, problem breakdown, and debugging notes."
header:
...
---
Merge Two Sorted Lists is a good example of turning a small-looking problem into a precise invariant. The Java implementation below captures the core idea, so this post focuses on why the approach works, what it is really solving, and how to debug it when the first attempt goes wrong.
...
Problem description:
We are given a problem around **Merge Two Sorted Lists** and need a Java solution that is correct on both normal and edge-case inputs. The challenge is usually not writing a loop or recursion call; it is preserving the one invariant that makes the approach valid from start to finish.
...
3. rely on recursion or subtree decomposition instead of value-copy shortcuts
...
- merge two sorted lists is a good pointer-discipline exercise because every link matters
...
public class MergeTwoSortedLists {
	...
	public ListNode mergeTwoListsRecursively(ListNode l1, ListNode l2) {
		...
		if (l1.val < l2.val) {
			handler = l1;
			handler.next = mergeTwoListsRecursively(l1.next, l2);
		} else {
			handler = l2;
			handler.next = mergeTwoListsRecursively(l1, l2.next);
		}
...
Dry-run Merge Two Sorted Lists with 3 to 5 nodes and explicitly draw the `next` links before and after each pointer update. That is the fastest way to catch lost-node bugs.
...
- Merge Two Sorted Lists becomes easier once the central invariant is stated in plain language.
